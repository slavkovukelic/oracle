...
def _legacy_package_names(packages):
    mapped = []
    seen = set()
    for pkg in packages:
        replacement = pkg
        if pkg.startswith("libnsl2"):
            suffix = pkg[len("libnsl2"):]
            replacement = "libnsl" + suffix
        elif pkg.startswith("python3-"):
            replacement = "python-" + pkg[len("python3-"):]
        elif pkg == "python3":
            replacement = "python"
        if not replacement:
            continue
        if replacement in seen:
            continue
        if replacement != pkg:
            LOG.debug("Using legacy package name %s instead of %s", replacement, pkg)
...
def _packages_for_plan(config):
    ...
    if os_major is not None and os_major < 7:
        packages = _legacy_package_names(packages)
        return packages
...
class PlanWriter(object):
    ...
    def apply_sysctl(self, params, sysctl_path):
        ...
        sysctl_path = PathHelper(sysctl_path)
        self.write_file(sysctl_path, header + body + "\n", mode=0o644)
        if not self.dry_run:
            run_command(["sysctl", "-p", sysctl_path.path], check=False)
...
def detect_oracle_linux_major_version(
    os_release_path=PathHelper("/etc/os-release"),
    redhat_release_path=PathHelper("/etc/redhat-release"),
):
    try:
...
def _parse_redhat_release(redhat_release_path):
    try:
