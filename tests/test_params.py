...
def test_calculate_hugepages_various_memory(mem_gb, swap_gb, expected):
    res = fake_resources(mem_gb, swap_gb=swap_gb)
    assert oracle_setup._calculate_hugepages(res) == expected
...
def test_legacy_runner_raises_on_failure(tmp_path):
    legacy_script = tmp_path / "oracle.sh"
    legacy_script.write_text("#!/bin/bash\nexit 1\n", encoding="utf-8")
    runner = oracle_setup.LegacyRunner(legacy_script)
    ...
    with mock.patch.object(oracle_setup, "ensure_root") as ensure_root_mock, mock.patch.object(
        ...
        ensure_root_mock.return_value = None
        run_mock.return_value = mock.Mock(returncode=1, args=["/bin/bash", str(legacy_script)], stdout="", stderr="boom")
        with pytest.raises(oracle_setup.subprocess.CalledProcessError):
            runner.execute(apply_changes=True, dry_run=False)
...
def test_ensure_root_rejects_non_privileged_user(monkeypatch):
    monkeypatch.setattr(oracle_setup.os, "geteuid", lambda: 1000)
    with pytest.raises(PermissionError):
        oracle_setup.ensure_root()
...
def test_system_inspector_reports_missing_meminfo(tmp_path, monkeypatch):
    inspector = oracle_setup.SystemInspector()
    monkeypatch.setattr(oracle_setup.SystemInspector, "MEMINFO_PATH", tmp_path / "missing")

    with pytest.raises(RuntimeError):
        inspector.collect()


def test_system_inspector_raises_on_invalid_meminfo(tmp_path, monkeypatch):
    inspector = oracle_setup.SystemInspector()
    meminfo_path = tmp_path / "meminfo"
    meminfo_path.write_text("MemTotal: notanint kB\n", encoding="utf-8")
    monkeypatch.setattr(oracle_setup.SystemInspector, "MEMINFO_PATH", meminfo_path)

    with pytest.raises(ValueError):
        inspector.collect()


def test_plan_writer_writes_expected_sysctl(tmp_path, monkeypatch):
    writer = oracle_setup.PlanWriter(dry_run=False)
    monkeypatch.setattr(oracle_setup, "run_command", lambda *args, **kwargs: None)
    params = {"kernel.shmmax": "123", "kernel.shmall": "456"}
    target = tmp_path / "99-oracle.conf"

    writer.apply_sysctl(params, target)

    content = target.read_text(encoding="utf-8").splitlines()
    assert content[0].startswith("# Generated by oracle_setup.py")
    assert "kernel.shmall = 456" in content[1:]
    assert "kernel.shmmax = 123" in content[1:]
